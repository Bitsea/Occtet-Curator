...
@Service
public class AILicenseMatcherWorkConsumer extends WorkConsumer {

    private static final Logger log = LogManager.getLogger(AILicenseMatcherWorkConsumer.class);
    ...
    protected void handleMessage(Message msg) {
        ...
        try {
            ...
            boolean result = workData.process(new BaseWorkDataProcessor() {
                @Override
                public boolean process(AILicenseMatcherWorkData aiLicenseMatcherWorkData) {
                    log.debug("go to generate");
                    return llmService.process(aiLicenseMatcherWorkData);
                }
            });
            if(!result){
